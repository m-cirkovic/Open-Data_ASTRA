...
export class PopUpService {
  ...
  public siteToHtml(site: Site): string {
    ...
    return `<h1>Richtung ${site.locationName}</h1><p>Geschwindigkeit: ${AverageService.getAvgVehicleSpeed(noError)} km/h</p><p>Anzahl Fahrzeuge pro Stunde: ${AverageService.getAvgVehicles(noError)}</p>${PopUpService.getErrorMsg(error)}`;
  }
